...
Object.assign(QueryCompiler_PG.prototype, {
  ...
  insert() {
    ...
    if (sql === '') return sql;
    const { returning, conflictColumns, conflictMergeData } = this.single;
    return {
      sql: sql + this._onConflict(conflictColumns, conflictMergeData) + this._returning(returning),
      returning,
    };
  },
  ...
  _onConflict(conflictColumns, conflictMergeData) {
    var sql = '';
    if (conflictColumns) {
      sql += ` on conflict`;
      if (conflictColumns.length) {
        sql += ` (${this.formatter.columnize(conflictColumns)})`;
      }
      if (conflictMergeData) {
        const updateData = this._prepUpdate(conflictMergeData);
        sql += ` do update set ${updateData.join(', ')}`;
      } else {
        sql += ' do nothing';
      }
    }
