...
Object.assign(QueryCompiler_MySQL.prototype, {
  ...
  insert() {
    ...
    if (sql === '') return sql;
    const { conflictMergeData } = this.single;
    return sql + this._onDuplicateKey(conflictMergeData);
  },
  ...
  _onDuplicateKey(conflictMergeData) {
    return conflictMergeData ? ` on duplicate key update ${this._prepUpdate(conflictMergeData).join(', ')}` : '';
  },
});
