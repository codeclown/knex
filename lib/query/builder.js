...
assign(Builder.prototype, {
  ...
  ignore(columns) {
    this._single.conflictMergeData = null;
    this._single.conflictColumns = columns || [];
    return this;
  },

  merge(updates, columns) {
    if (!isObject(updates)) {
      throw new Error('merge() first argument must be an object');
    }
    if (typeof columns !== 'undefined' && !Array.isArray(columns)) {
      throw new Error('merge() second argument must be an array');
    }
    this._single.conflictMergeData = updates;
    this._single.conflictColumns = columns || [];
    return this;
  },
